...
class HTree(object):
    ...
    def __init__(self, arity, leaf_counts):
        ...
        assert arity**(height-1) == len(leaf_counts), \
            'Invalid number of leaves, %d, must be power of %d' % (
                len(leaf_counts), arity)
        ...
        while len(nodes) > 1:
            ...
            del nodes[:self.k]
            total = reduce(lambda x, y: x + y.count, siblings,
                           0)  # sum up counts of siblings
    ...
    def inference_no_consistency(self):
        k = self.k
        ...
        for (node, h) in self.postorder_iter(with_height=True):
            if node.isleaf():
                ...
                total = reduce(lambda total, child: total +
                               child.hbar, node.children, 0)
